...
function configureCollection(store, rsshub, rsshubRoot) {
...
test('真实采集链路：RSS HTTP 503 会使任务失败并阻断批次', async () => {
  ...
  try {
    ...
    const source = configureCollection(store, rsshub, rsshubRoot);
...
test('真实文章链路：模型 HTTP 502 会使 AI 任务失败且不产生完成态', async () => {
  ...
  try {
    ...
    configureCollection(store, rsshub, rsshubRoot);
