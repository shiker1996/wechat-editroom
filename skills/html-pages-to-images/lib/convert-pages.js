...
export async function convertPagesToImages(options = {}) {
  const {
    ...
    deviceScaleFactor = 3,
    selector = '.page',
    /** 可选：加载后等待 window 上某全局为 true（如 ECharts 初始化完成） */
    waitForGlobalTruthy = null,
    waitForGlobalTimeout = 15000,
  ...
  try {
    ...
    const expectedViewportPixels = {
      width: pageWidth * deviceScaleFactor,
      height: pageHeight * deviceScaleFactor,
    };

    console.log(`📐 deviceScaleFactor: ${deviceScaleFactor}`);
    console.log(`🧮 预期视口像素: ${expectedViewportPixels.width}x${expectedViewportPixels.height}`);
    ...
    if (waitForGlobalTruthy) {
      const expr = `window[${JSON.stringify(waitForGlobalTruthy)}] === true`;
      await page.waitForFunction(expr, { timeout: waitForGlobalTimeout }).catch(() => {});
    ...
    for (let i = 0; i < pageElements.length; i++) {
      ...
      const pageInfo = await page.evaluate((el) => {
        // 优先使用 data-page-number 属性（用于封面页）
        let pageNumber = el.getAttribute('data-page-number') || '';
        // 如果没有 data-page-number，使用 .page-number 元素
        if (!pageNumber) {
          pageNumber = el.querySelector('.page-number')?.textContent?.trim() || '';
        }
