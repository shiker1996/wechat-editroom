...
## 与 `wechat-mp-topic-to-article` 的衔接

| 项 | 说明 |
|----|------|
| **编排 Step** | **Step 4**：读 **`03-draft.md`** → 写 **`04-humanized.md`**（`{WORKDIR}`，非空落盘） |
| **与 `wechat-mp-*` 直连** | 成稿全链须由编排器调用本步；**勿**在编排模式下由写作子技能再单独调本技能 |
| **⚠️ 质量闸门** | 本技能输出须达到最低门槛方可交付；不合格品**打回重写**，不得绕过 |
...
6. **保留 Markdown 结构** - 所有标题行（`#`、`##`、`###`）原样保留，尤其是首行 `# 一级标题`：不改写标题文字、不删除井号、不把标题改成加粗段落或普通句子
...
5. **保留有效句子** - 不主动删除金句、反差、人物细节或有传播力的判断；只有在句子空泛、夸张或明显像模板时才重写

## 流量文保护规则

公众号流量文的自然化不是把文章改成中性说明文。处理 `wechat-mp-topic-to-article` 的稿件时：

- 保留标题承诺、前 200 字的冲突、作者明确立场和结尾余味。
- 保留能推动完读的短句、反差、停顿、人物细节和情绪变化。
- 不主动删除“像金句”的句子，不把锋利判断改成空泛的平衡表达。
- 只修复 AI 腔、重复、机械排比、虚假宏大意义、模糊归因和不自然的连接词。
- 不新增事实、引语、数字、来源、经历或读者行动建议。
...
- ✓ **首行 `# 一级标题` 和各级标题行原样保留？** 标题不是 AI 痕迹，禁止改写或去掉 `#`
- ✓ **连续三个句子长度相同？** 打断其中一个
- ✓ **段落以简洁的单行结尾？** 变换结尾方式
- ✓ **揭示前有破折号？** 删除它
- ✓ **解释隐喻或比喻？** 相信读者能理解
...
**硬拦截（强制闸门，不得绕过）：**
- **总分 < 35** → **打回重写**，在产物中注明「AI痕迹过重」，不得交付给下一步
- **单项维度（真实性/节奏）< 5** → 针对性重写对应段落至达标
- **AI特征词 > 5 处** → 全面返工，不接受部分修改

**标准：**
- 45-50 分：优秀，已去除 AI 痕迹
- 35-44 分：良好，仍有改进空间（须按审阅意见修改至 45+ 方可交付）
- 低于 35 分：打回重写
