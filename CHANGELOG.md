...
- 交互型 Agent（编辑室、自主写作和自定义图文）共享更宽松但仍受硬上限约束的执行预算：默认最多 6 个模型步骤、10 次工具调用、64,000 字符工具结果和 180 秒超时；任务日志可通过 Run Trace 定位模型与工具链路。

### Fixed

- 网页 URL 抓取成功后回填资源目录正文，后续段落检索可通过 `resourceIds` 找到同一份已抓取资料。
- 编辑室 Agent 的流式失败会通过页面错误事件显示原因；达到预算或非最终结束时保留可见的助手提示，不再出现空白回复。
...
- **Run Trace 输入可观测性**：任务日志可查看模型调用输入、模型与工具链路，并支持按调用定位完整运行轨迹。

### Changed

- 工具 Schema 和字段校验失败统一返回字段路径、期望规则和可执行修正提示，降低模型重复提交同一错误的概率。
- 模型配置、连接测试统一收口到“运行与配置”，模型调用记录统一在“任务日志”查看。
