...
import { AI_THEME_ERROR_CODES, AiThemeContractError, composeAiThemeDefinition, validateAiThemeRequest } from '../../../shared/themes/ai-theme-contract.mjs';
import { AiThemeCandidateStore } from '../../../shared/themes/ai-theme-candidate-store.mjs';
import { THEME_RECIPE_CATALOG, themeRecipeEditorCatalog } from '../../../shared/themes/recipe-catalog.mjs';
...
export function normalizeAiThemeCandidate(input,{target}){
  ...
  if(!['sans','serif','song','kai','hei','mono'].includes(typography.family))repair(state,'tokens.typography.family',typography.family,targetDefaults.typography.family,'回退到受支持的正文字体角色');
  if(!['sans','serif','song','kai','hei','mono'].includes(typography.headingFamily))repair(state,'tokens.typography.headingFamily',typography.headingFamily,targetDefaults.typography.headingFamily,'回退到受支持的标题字体角色');
...
export async function generateAiThemeCandidate({gateway,input,candidateStore=new AiThemeCandidateStore(),signal,referenceThemes=[]}={}){
  ...
  let composed;try{composed=composeAiThemeDefinition(normalized.candidate,{target:request.target,id:temporaryId,templateMatchContext:{preferences:request.preferences}});}catch(error){if(error instanceof AiThemeContractError)throw error;throw new AiThemeContractError(AI_THEME_ERROR_CODES.MODEL_OUTPUT_INVALID,error.message,error.issues||[]);}
