...
import { checkReddit } from '../../../../plugins/reddit/collector.mjs';
import { testFeedSubscription } from '../../../../plugins/feed/collector.mjs';
import { checkRssHub, ensureStarted, stopRssHub, testSubscription } from '../../../../plugins/rsshub/collector.mjs';
import {
  addSubscription,
  listSubscriptions,
  removeSubscription,
  ...
  updateSubscription,
} from '../../../features/collection/application/subscriptions.mjs';
...
import { readAgentConsumers } from '../../tools/dependency-baseline.mjs';
import { assertAuthorizationChange, describeSkillAuthorization, previewSkillAuthorizationChange, saveSkillAuthorization } from '../../skills/capability-authorization.mjs';
...
import { invalidateCapabilityHealthCache, prefetchCapabilityHealth } from '../../tools/health-check.mjs';
import { migrateLegacyCapabilityRoutes, readCapabilityRoutes, setCapabilityRoute } from '../../tools/capability-routes.mjs';
import { addCapabilityCatalogEntries, catalogDraftsForManifest, findUnregisteredCapabilities } from '../../tools/capability-catalog.mjs';
import { ExtensionConfigurationService, LEGACY_CONFIGURATION_FALLBACK_REMOVAL_VERSION } from '../../extensions/configuration-service.mjs';
...
export async function handleSystemRoutes(context) {
  ...
  const capabilityGraph=async()=>{
    ...
    // 遗留 6：构建前并发预取真实健康表（进程内 TTL 缓存），图谱据此覆盖 implementationHealthy 代理
    const capabilityIds=[...new Set(registry.listCapabilities({includeDisabled:true}).map((item)=>item.capability))];
    const healthByCapability=await prefetchCapabilityHealth(registry,capabilityIds);
    return buildCapabilityGraph({root,tools:[...toolsById.values()],collectors,collectionSources:store.listCollectionSources(),routes:readCapabilityRoutes(root).routes,healthByCapability,configurationState:(type,id,manifest)=>extensionConfiguration.describe({extensionType:type,extensionId:id,manifest})});
  };
  ...
  // R2 顺序规则收口：目录外（registered:false）能力的实现允许存在（调试期宽容），但不得启用、不得设为路由首选
  const rejectUnregistered=(capabilityIds,action)=>{
    const unregistered=findUnregisteredCapabilities(root,capabilityIds);
    if(!unregistered.length)return false;
  ...
  if(request.method==='GET'&&pathname==='/api/system/configuration/catalog'){
    const items=await configurationCatalog();json(response,200,{items:items.map(({manifest,...item})=>({...item,renderer:manifest.configurationRenderer||'standard',state:describeResource({...item,manifest})}))});return true;
  }
  if(request.method==='GET'&&pathname==='/api/system/configuration/migration-status'){
    ...
    json(response,200,{total:resources.length,migrated,legacyFallback,defaults:resources.length-migrated-legacyFallback,coverage:resources.length?Number((migrated/resources.length*100).toFixed(1)):100,readyToDisableFallback:legacyFallback===0,legacyFallbackRemovalVersion:LEGACY_CONFIGURATION_FALLBACK_REMOVAL_VERSION,resources});return true;
  }
  ...
  if(request.method==='GET'&&pathname==='/api/system/capability-consumers'){
    const graph=await capabilityGraph();
    const consumers=graph.consumers.map((consumer)=>{
      const states=graph.consumerStates.filter((state)=>state.consumerId===consumer.id);
      return {consumerId:consumer.id,name:consumer.name,type:consumer.type,entryPoint:consumer.entryPoint||'',runtimeSkillIds:consumer.runtimeSkillIds||[],purpose:consumer.purpose||'',enabled:consumer.enabled!==false,
        summary:{total:states.length,available:states.filter((state)=>state.available).length,degraded:states.filter((state)=>state.status==='degraded').length,blocked:states.filter((state)=>!state.available).length}};
    });
    json(response,200,{schemaVersion:1,consumers});return true;
  }
  const capabilityConsumerMatch=pathname.match(/^\/api\/system\/capability-consumers\/([^/]+)$/);
  if(request.method==='GET'&&capabilityConsumerMatch){
    const consumerId=decodeURIComponent(capabilityConsumerMatch[1]),graph=await capabilityGraph();
    const consumer=graph.consumers.find((item)=>item.id===consumerId);
    if(!consumer){json(response,404,{error:'消费者不存在'});return true;}
    // 扩展方案阶段 C（§5.1）：无归属入口的技能消费者授权载体即其自身；feature 消费者无授权开关（空数组属预期）
    const runtimeSkillIds=consumer.type==='skill'?[consumer.id]:(consumer.runtimeSkillIds||[]);
    const authorizations=runtimeSkillIds.map((skillId)=>{
      const description=describeSkillAuthorization(root,graph,skillId);
      return {skillId,whitelist:description.whitelist,editable:description.editable,locked:description.locked,version:description.version,integrity:description.integrity};
    });
    json(response,200,{schemaVersion:1,consumerId,name:consumer.name,type:consumer.type,entryPoint:consumer.entryPoint||'',runtimeSkillIds,adapterModule:consumer.adapterModule||'',purpose:consumer.purpose||'',enabled:consumer.enabled!==false,adaptation:consumer.adaptation||null,
      skillAuthorizations:authorizations,
      capabilities:graph.consumerStates.filter((state)=>state.consumerId===consumerId).map((state)=>({...state,
        skillAuthorizations:authorizations.map((entry)=>({skillId:entry.skillId,version:entry.version,
          allowed:!entry.whitelist||entry.whitelist.includes(state.capability),
          editable:state.declared&&entry.editable.includes(state.capability),
          lockedReason:entry.locked.find((item)=>item.capability===state.capability)?.reason||''}))}))});return true;
  }
  const capabilityRouteMatch=pathname.match(/^\/api\/system\/capability-routes\/(.+)$/);
  // R3 目录草案确认入库：人工确认（管理员）后写入 config/capabilities.json，校验复用目录读取的条目规则
  if(request.method==='POST'&&pathname==='/api/system/capability-catalog'){
    try{requirePluginAdmin(request);const input=await body(request);const added=addCapabilityCatalogEntries(root,input.entries);json(response,200,{registered:added.map((item)=>item.id),entries:added});}
    catch(error){json(response,400,{error:error.message});}
    return true;
  }
  if(request.method==='PUT'&&capabilityRouteMatch){const capability=decodeURIComponent(capabilityRouteMatch[1]),input=await body(request),pluginId=String(input.preferredImplementationId||'');const registry=await getToolRegistry();if(pluginId&&!registry.listCapabilities({includeDisabled:true}).some((item)=>item.capability===capability&&item.plugin===pluginId)){json(response,400,{error:'所选工具不实现该能力'});return true;}if(pluginId&&rejectUnregistered([capability],'设为路由首选'))return true;json(response,200,setCapabilityRoute(root,capability,{preferredImplementationId:pluginId}));return true;}
  ...
  if(unifiedConfigurationMatch&&['GET','PUT'].includes(request.method)){
    ...
    try{
      ...
      if(request.method==='GET'){
        ...
        if(type==='model-provider'){
          const stored=extensionSettingRepository.get(type,id);
          if(stored&&stored.value&&'default'in stored.value){
            const clean={...stored.value};delete clean.default;
            extensionSettingRepository.save({extensionType:type,extensionId:id,scope:stored.scope,schemaVersion:stored.schema_version,value:clean,configured:stored.configured,status:stored.status,configHash:stored.config_hash});
          }
          result=describeResource(resource);
        }
      }
      json(response,200,result);
    }catch(error){json(response,400,{error:error.message,issues:error.issues||[]});}return true;
  }
  const unifiedConfigurationTestMatch=pathname.match(/^\/api\/system\/configuration\/([^/]+)\/([^/]+)\/test$/);
  if(request.method==='POST'&&unifiedConfigurationTestMatch){const type=decodeURIComponent(unifiedConfigurationTestMatch[1]),id=decodeURIComponent(unifiedConfigurationTestMatch[2]);const resource=findConfigurationResource(await configurationCatalog(),type,id);if(!resource){json(response,404,{error:'配置资源不存在'});return true;}const state=describeResource(resource);json(response,state.configured?200:400,{pass:state.configured,state});return true;}
  if (request.method === 'GET' && pathname === '/api/system/skills') {
    ...
    const loadedTools=await Promise.all(toolRegistry.listCapabilities({includeDisabled:true})
      .filter((item)=>pluginCatalog.plugins[item.plugin]?.status!=='uninstalled')
      .map(async(item)=>({
    ...
    const installedTools=Object.values(pluginCatalog.plugins).filter((item)=>item.status!=='uninstalled').flatMap((item)=>
      item.manifest.capabilities.map((capability)=>({
        ...
        health:{status:'ok',data:{available:false,disabled:item.status!=='enabled',restartRequired:Boolean(item.restartRequired)}},
  ...
  if(request.method==='GET'&&pathname==='/api/system/tool-plugin-install-events'){
    json(response,200,{items:listToolPluginInstallEvents(root,boundedLimit(searchParams,100,500))});return true;
  }
  if(request.method==='GET'&&pathname==='/api/system/remote-tool-plugin-events'){
    json(response,200,{items:listRemotePluginEvents(root,boundedLimit(searchParams,100,500))});return true;
  }
  if(request.method==='POST'&&['/api/system/remote-tool-plugins/validate','/api/system/remote-tool-plugins'].includes(pathname)){
    try{
      if(!pathname.endsWith('/validate'))requirePluginAdmin(request);
      ...
      const result=pathname.endsWith('/validate')?checked:installRemotePlugin(root,checked,{expectedManifestHash:String(input.expectedManifestHash||'')});
      if(!pathname.endsWith('/validate'))await reloadToolRegistry();
      // R3：声明了目录外能力时附目录条目草案（保守占位），需人工确认后经 POST /api/system/capability-catalog 入库
      json(response,200,{...result,catalogDrafts:catalogDraftsForManifest(root,checked)});
    }catch(error){json(response,400,{error:error.message});}
    return true;
  }
  const remoteStatusMatch=pathname.match(/^\/api\/system\/remote-tool-plugins\/([^/]+)\/status$/);
  if(request.method==='PATCH'&&remoteStatusMatch){
    try{requirePluginAdmin(request);const id=decodeURIComponent(remoteStatusMatch[1]),input=await body(request);if(input.status==='enabled'){const manifest=readRemotePluginCatalog(root).plugins[id]?.manifest;if(manifest&&rejectUnregistered(manifest.capabilities||[],'启用实现'))return true;}if(input.status==='disabled'){const check=await validateDisableImpact('tool',id,input);if(!check.ok){json(response,check.status,{error:check.error,...check});return true;}}const result=setRemotePluginStatus(root,id,input.status);await reloadToolRegistry();json(response,200,{...result,appliesTo:'new-tasks'});}
    catch(error){json(response,400,{error:error.message});}
    return true;
  }
  ...
  if(request.method==='POST'&&remoteFirstRunMatch){
    try{requirePluginAdmin(request);const result=confirmRemotePluginFirstRun(root,decodeURIComponent(remoteFirstRunMatch[1]));json(response,200,result);}
    catch(error){json(response,400,{error:error.message});}
    return true;
  }
  ...
  if(request.method==='DELETE'&&remoteDeleteMatch){
    ...
    if(!item||item.status==='uninstalled'){json(response,404,{error:'远程插件不存在'});return true;}
    try{
      requirePluginAdmin(request);const input=await body(request),check=await validateDisableImpact('tool',id,input);if(!check.ok){json(response,check.status,{error:check.error,...check});return true;}
  ...
  if(request.method==='POST'&&['/api/system/tool-plugin-packages/validate','/api/system/tool-plugin-packages/install'].includes(pathname)){
    try{
      ...
      const result=pathname.endsWith('/validate')?validateToolPluginDirectory(input.directory)
        :installToolPlugin({workspaceRoot:root,directory:input.directory,builtinIds:BUILTIN_PLUGINS,expectedContentHash:String(input.expectedContentHash||'')});
      // R3：声明了目录外能力时附目录条目草案（保守占位），需人工确认后经 POST /api/system/capability-catalog 入库
      const safe={...result,catalogDrafts:catalogDraftsForManifest(root,result.manifest)};delete safe.directory;
  ...
  if(request.method==='PATCH'&&installedPluginStatusMatch){
    try{requirePluginAdmin(request);const id=decodeURIComponent(installedPluginStatusMatch[1]),input=await body(request);if(input.status==='enabled'){const manifest=readToolPluginCatalog(root).plugins[id]?.manifest;if(manifest&&rejectUnregistered(manifest.capabilities||[],'启用实现'))return true;}if(input.status==='disabled'){const check=await validateDisableImpact('tool',id,input);if(!check.ok){json(response,check.status,{error:check.error,...check});return true;}}json(response,200,{...setInstalledToolPluginStatus(root,id,input.status),appliesTo:'new-tasks'});}
    catch(error){json(response,400,{error:error.message});}
    return true;
  }
  ...
  if (request.method === 'GET' && pathname === '/api/system/skill-install-events') {
    json(response,200,{items:listSkillInstallEvents(root,boundedLimit(searchParams,100,500))});return true;
  }
  ...
  if(request.method==='DELETE'&&toolPluginMatch){
    ...
    try{
      ...
      const result=uninstallToolPlugin(root,pluginId);
      await reloadToolRegistry();
      json(response,200,{...result,impact:check.impact,appliesTo:'new-tasks',historyPreserved:true});
    }catch(error){json(response,400,{error:error.message});}
    return true;
  }
  if(request.method==='PATCH'&&toolPluginMatch){
    ...
    requirePluginAdmin(request);const input=await body(request);
    ...
    if(input.enabled===true&&rejectUnregistered(plugin.capabilities||[],'启用实现'))return true;
  ...
  if(extensionConfigurationMatch&&['GET','PUT'].includes(request.method)){
    ...
    // 阶段 4b：技能能力授权走 active.json 的 capabilityAuthorization（含版本协商与影响预览），
    // 与动态配置 schema（ExtensionConfigurationService）是两条独立写入路径
    const input=request.method==='PUT'?await body(request):null;
    if(extensionType==='skill'&&input&&typeof input==='object'&&'capabilityAuthorization' in input){
      const graph=await capabilityGraph();
      const capabilities=input.capabilityAuthorization===null?null:input.capabilityAuthorization?.capabilities??null;
      try{
        if(input.dryRun){
          const description=assertAuthorizationChange(root,graph,extensionId,capabilities);
          json(response,200,{dryRun:true,...description,impact:previewSkillAuthorizationChange(root,graph,{skillId:extensionId,capabilities}).changes});
        }else{
          json(response,200,saveSkillAuthorization(root,graph,extensionId,{capabilities,expectedVersion:input.expectedVersion}));
        }
      }catch(error){
        const status=['CONFIG_VERSION_CONFLICT','CONFIG_INTEGRITY_BROKEN'].includes(error.code)?409:400;
        json(response,status,{error:error.message,code:error.code||'CAPABILITY_AUTHORIZATION_INVALID',issues:error.issues||[],currentVersion:error.currentVersion});
      }
      return true;
    }
    try{
      const result=request.method==='GET'
        ?(extensionType==='skill'&&readAgentConsumers(root).some((consumer)=>(consumer.runtimeSkillIds||[]).includes(extensionId))
          ? capabilityGraph().then((graph)=>({...extensionConfiguration.describe({extensionType,extensionId,manifest}),capabilityAuthorization:describeSkillAuthorization(root,graph,extensionId)}))
          : extensionConfiguration.describe({extensionType,extensionId,manifest}))
        :extensionConfiguration.save({extensionType,extensionId,manifest,input});
  ...
  if (request.method === 'GET' && pathname === '/api/system/generation-snapshots') {
    json(response, 200, { items:store.listGenerationSnapshots({
      batchId:searchParams.get('batchId') || null,
      candidateId:searchParams.get('candidateId') ? Number(searchParams.get('candidateId')) : null,
      limit:boundedLimit(searchParams,50,500),
    }) });
    return true;
  }
  ...
  if (request.method === 'GET' && pathname === '/api/subscriptions') {
    if(store?.repositories?.collectionSources)json(response, 200, listSubscriptions(store.repositories.collectionSources, store.listSubscriptionHealth()));
    else json(response, 200, { items: [], summary: { total: 0, enabled: 0 } });
    return true;
  }
  ...
  if(request.method==='PATCH'&&collectorRuntimeSettingMatch){requirePluginAdmin(request);const id=decodeURIComponent(collectorRuntimeSettingMatch[1]),input=await body(request);if(input.enabled===true){const collector=listCollectorPluginStates(root,createBuiltinCollectorRegistry().list()).find((item)=>item.id===id);if(collector&&rejectUnregistered(collector.capabilities||[],'启用实现'))return true;}if(input.enabled===false){const check=await validateDisableImpact('collector',id,input);if(!check.ok){json(response,check.status,{error:check.error,...check});return true;}}invalidateCapabilityHealthCache();json(response,200,{...writeCollectorToolSetting(root,id,input),appliesTo:'new-tasks'});return true;}
  if(request.method==='GET'&&pathname==='/api/system/collector-plugin-events'){json(response,200,{items:listCollectorPluginEvents(root,boundedLimit(searchParams,100,500))});return true;}
  const collectorVersionsMatch=pathname.match(/^\/api\/system\/collector-plugins\/([^/]+)\/versions$/);
  if(request.method==='GET'&&collectorVersionsMatch){json(response,200,{items:listCollectorPluginVersions(root,decodeURIComponent(collectorVersionsMatch[1]))});return true;}
  const collectorRollbackMatch=pathname.match(/^\/api\/system\/collector-plugins\/([^/]+)\/rollback$/);
  if(request.method==='POST'&&collectorRollbackMatch){try{requirePluginAdmin(request);json(response,200,rollbackCollectorPlugin(root,decodeURIComponent(collectorRollbackMatch[1]),(await body(request)).version));}catch(error){json(response,400,{error:error.message});}return true;}
  ...
  if(request.method==='PATCH'&&collectorStatusMatch){try{requirePluginAdmin(request);const id=decodeURIComponent(collectorStatusMatch[1]),input=await body(request);if(input.status==='enabled'){const collector=listCollectorPluginStates(root,createBuiltinCollectorRegistry().list()).find((item)=>item.id===id);if(collector&&rejectUnregistered(collector.capabilities||[],'启用实现'))return true;}if(input.status==='disabled'){const check=await validateDisableImpact('collector',id,input);if(!check.ok){json(response,check.status,{error:check.error,...check});return true;}}invalidateCapabilityHealthCache();json(response,200,{...setCollectorPluginStatus(root,id,input.status),appliesTo:'new-tasks'});}catch(error){json(response,400,{error:error.message});}return true;}
  const collectorConfirmMatch=pathname.match(/^\/api\/system\/collector-plugins\/([^/]+)\/first-run-confirm$/);
  if(request.method==='POST'&&collectorConfirmMatch){try{requirePluginAdmin(request);json(response,200,confirmCollectorPluginFirstRun(root,decodeURIComponent(collectorConfirmMatch[1])));}catch(error){json(response,400,{error:error.message});}return true;}
  ...
  if (request.method === 'POST' && pathname === '/api/system/backup/restore') {
    if (!request.localSecurity?.consume(request, 'backup-restore')) {
  ...
  if (request.method === 'GET' && pathname === '/api/subscriptions/health-history') {
    json(response, 200, store.listSubscriptionHealthHistory({
      ...
      limit: boundedLimit(searchParams,500,500),
    }));
    return true;
  }
  if (request.method === 'POST' && pathname === '/api/subscriptions/test') {
    const input = subscriptionTestInput(await body(request));
    json(response, 200, input.kind === 'direct'
      ? await testFeedSubscription(input.value || input.url)
      : await testSubscription(config.rsshub, input));
    return true;
  }
  if (request.method === 'POST' && pathname === '/api/subscriptions') {
    if(!store?.repositories?.collectionSources){json(response,500,{error:'数据源存储不可用'});return true;}
    try{addSubscription(store.repositories.collectionSources, await body(request));json(response, 201, listSubscriptions(store.repositories.collectionSources, store.listSubscriptionHealth()));}
    catch(error){json(response,400,{error:error.message});}
    return true;
  }
  if (request.method === 'PATCH' && pathname === '/api/subscriptions') {
    if(!store?.repositories?.collectionSources){json(response,500,{error:'数据源存储不可用'});return true;}
    try{updateSubscription(store.repositories.collectionSources, await body(request));json(response, 200, listSubscriptions(store.repositories.collectionSources, store.listSubscriptionHealth()));}
    catch(error){json(response,400,{error:error.message});}
    return true;
  }
  if (request.method === 'DELETE' && pathname === '/api/subscriptions') {
    if(!store?.repositories?.collectionSources){json(response,500,{error:'数据源存储不可用'});return true;}
    try{removeSubscription(store.repositories.collectionSources, await body(request));json(response, 200, listSubscriptions(store.repositories.collectionSources, store.listSubscriptionHealth()));}
    catch(error){json(response,400,{error:error.message});}
    return true;
  }
  return false;
