...
import { invalidateCapabilityHealthCache } from './health-check.mjs';
...
function discoverPluginIds(directory){
  ...
  return fs.readdirSync(directory,{withFileTypes:true})
    .filter((entry)=>{
      if(!entry.isDirectory()||entry.name==='shared')return false;
      const manifestFile=path.join(directory,entry.name,'manifest.json');
      if(!fs.existsSync(manifestFile))return false;
      return JSON.parse(fs.readFileSync(manifestFile,'utf8')).kind==='tool';
    })
