...
const defaults = {
  ...
  rsshub: {
    ...
    rootDir: 'RSSHub',
    startScript: 'scripts/runtime/rsshub-start.ps1',
  ...
  githubDiscovery:{enabled:true,createdWithinDays:30,activeSearchEnabled:true,activeWithinDays:30,activeMinStars:100,evergreenSearchEnabled:true,evergreenWithinDays:180,evergreenMinStars:500,minStars:1000,limit:30,cacheTtlMs:1800000,
...
export function resolveArticleLength(config, chain) {
  ...
  if(!Number.isFinite(min)||!Number.isFinite(max)||min<0||max<=0||min>max)throw new Error(`文章字数配置无效：minVisibleChars=${String(override.minVisibleChars??base.minVisibleChars)}, maxVisibleChars=${String(override.maxVisibleChars??base.maxVisibleChars)}`);
  return { min, max };
}
