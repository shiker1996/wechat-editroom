...
const SCENARIO_PATTERNS = Object.freeze({
  'file-content': /pdf|markdown|md|ocr|document|file|text extraction|knowledge base|知识库|文档|文件|转换|文本/i,
...
function normalizeType(value) {
  ...
  const aliases = {
    cli: 'tool',
    'desktop-tool': 'tool',
    'browser-extension': 'plugin-extension',
    library: 'component',
    sdk: 'component',
...
export function normalizeGitHubRepository(value) {
  ...
  return owner && repo && !['topics', 'trending', 'search', 'marketplace'].includes(owner.toLowerCase())
    ? `${owner}/${repo}`.toLowerCase()
    : '';
}

export function repositoryKey(url, rawJson = '') {
  let raw = {};
  try { raw = JSON.parse(rawJson || '{}'); } catch {}
  const declared = normalizeGitHubRepository(raw.repository);
  if (declared) return declared;
  try {
    const parsed = new URL(String(url || ''));
    if (parsed.hostname.toLowerCase() !== GITHUB_HOST) return '';
    return normalizeGitHubRepository(parsed.pathname);
  } catch { return ''; }
}

export function inferGitHubScenarioIds({ lane = '', query = '', label = '', description = '', topics = [], language = '' } = {}) {
  const text = textOf([lane, query, label, description, topics, language]);
  const result = [];
  if (GITHUB_SCENARIO_LANES.includes(String(lane))) result.push(String(lane));
  for (const [scenario, pattern] of Object.entries(SCENARIO_PATTERNS)) {
    if (pattern.test(text)) result.push(scenario);
  }
...
export function classifyGitHubRepository(repository = {}, querySpec = {}) {
  const text = textOf([
    ...
    querySpec.label,
    querySpec.query,
    querySpec.directUseCase,
  ]);
  const scenarios = [...new Set([
    ...(Array.isArray(repository.scenarioIds) ? repository.scenarioIds : []),
    ...(Array.isArray(querySpec.scenarioIds) ? querySpec.scenarioIds : []),
    ...inferGitHubScenarioIds({ lane: querySpec.lane, query: querySpec.query, label: querySpec.label, description: repository.description, topics: repository.topics, language: repository.language }),
  ...
  const classificationEvidence = querySpec.lane || querySpec.projectTypes?.length ? 'query-lane' : repository.description || repository.topics?.length ? 'metadata' : 'uncertain';
...
export function extractScenarioIds(value = {}) {
  const raw = value?.scenarioIds || value?.scenario_ids || value?.githubScenarioIds || value?.github_scenario_ids || value?.repositoryMeta?.scenarioIds;
  return Array.isArray(raw) ? raw.filter((item) => GITHUB_SCENARIO_LANES.includes(String(item))) : [];
}
