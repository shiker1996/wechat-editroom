...
const TEMPLATE_MATCH_CONFIDENCE=new Set(['high','medium','low']);
const TEMPLATE_MATCH_REASON_CODES=new Set(['CLEAR_DIRECTION','NO_DIRECTION_SIGNAL','WEAK_DIRECTION_SIGNAL','AMBIGUOUS_DIRECTION_SIGNAL']);
...
export function validateThemeDefinition(input,{filePath='',expectedTarget=null,expectedSource=null,enforceNumericSteps=false}={}){
  ...
  for(const target of ['article','social']){
    ...
    if(!config)continue;
    unknown(config,new Set(target==='article'?['recipes','behavior','components']:['recipes','effects','components','templatePack','templateMatch']),target,issues);
    ...
    if(target==='social'&&config.templatePack!==undefined){
      const value=config.templatePack;
      if(!value||typeof value!=='object'||Array.isArray(value))issues.push({field:'social.templatePack',code:'TYPE',message:'必须是包含 id/version 的对象'});
      else {
        const custom=Boolean(value.roles&&value.roleTemplates);
        unknown(value,custom?new Set(['id','version','label','renderer','proposalId','source','fallbackTemplate','css','surface','roleTemplates','roles']):new Set(['id','version']),'social.templatePack',issues);
        if(custom){
          if(!/^proposal-[a-z0-9-]+-v\d+$/.test(String(value.id||'')))issues.push({field:'social.templatePack.id',code:'FORMAT',message:'自定义模板包 ID 必须是 proposal-*-vN 格式'});
          if(value.source!=='proposal')issues.push({field:'social.templatePack.source',code:'ENUM',message:'自定义模板包来源必须为 proposal'});
          const roleKeys=new Set(['cover','concept','feature','steps','data','compare','evidence','timeline','risk','ending']);
          if(!value.roleTemplates||[...roleKeys].some((role)=>typeof value.roleTemplates[role]!=='string'))issues.push({field:'social.templatePack.roleTemplates',code:'ROLE_COVERAGE',message:'自定义模板包必须覆盖十个页面角色'});
          if(!value.roles||[...roleKeys].some((role)=>!value.roles[role]))issues.push({field:'social.templatePack.roles',code:'ROLE_COVERAGE',message:'自定义模板包必须提供十个角色能力'});
          if(value.css!==undefined&&typeof value.css!=='string')issues.push({field:'social.templatePack.css',code:'TYPE',message:'模板包 CSS 必须是字符串'});
        }
        const pack=getSocialCardTemplatePack(value.id);
        if(!pack&&!custom)issues.push({field:'social.templatePack.id',code:'ENUM',message:'模板包不存在'});
        else if(pack&&!custom&&value.version!==pack.version)issues.push({field:'social.templatePack.version',code:'VERSION_MISMATCH',message:`必须为 ${pack.version}`});
      }
    }
    if(target==='social'&&config.templateMatch!==undefined){
      const value=config.templateMatch;
      if(!value||typeof value!=='object'||Array.isArray(value))issues.push({field:'social.templateMatch',code:'TYPE',message:'必须是程序生成的模板匹配摘要'});
      else {
        unknown(value,new Set(['schemaVersion','packId','source','confidence','reasonCode','score','runnerUpScore','margin','reason','signals']),'social.templateMatch',issues);
        if(value.schemaVersion!==1)issues.push({field:'social.templateMatch.schemaVersion',code:'UNSUPPORTED_SCHEMA',message:'必须为 1'});
        if(!ID.test(value.packId||''))issues.push({field:'social.templateMatch.packId',code:'FORMAT',message:'必须是合法模板包 ID'});
        if(!TEMPLATE_MATCH_SOURCES.has(value.source))issues.push({field:'social.templateMatch.source',code:'ENUM',message:'模板匹配来源不受支持'});
        if(!TEMPLATE_MATCH_CONFIDENCE.has(value.confidence))issues.push({field:'social.templateMatch.confidence',code:'ENUM',message:'模板匹配置信度不受支持'});
        if(!TEMPLATE_MATCH_REASON_CODES.has(value.reasonCode))issues.push({field:'social.templateMatch.reasonCode',code:'ENUM',message:'模板匹配低置信度原因不受支持'});
        for(const field of ['score','runnerUpScore'])if(!Number.isInteger(value[field])||value[field]<0||value[field]>100)issues.push({field:`social.templateMatch.${field}`,code:'RANGE',message:'模板匹配分数必须是 0–100 的整数'});
        if(!Number.isInteger(value.margin)||value.margin<-100||value.margin>100)issues.push({field:'social.templateMatch.margin',code:'RANGE',message:'模板匹配分差必须是 -100–100 的整数'});
        if(typeof value.reason!=='string'||!value.reason.trim()||[...value.reason].length>240)issues.push({field:'social.templateMatch.reason',code:'LENGTH',message:'匹配理由必须为 1–240 个字符'});
        if(!Array.isArray(value.signals)||value.signals.length>8||value.signals.some((item)=>typeof item!=='string'||!item.trim()||[...item].length>40))issues.push({field:'social.templateMatch.signals',code:'FORMAT',message:'匹配信号必须是不超过 8 项的短字符串数组'});
        const pack=getSocialCardTemplatePack(value.packId),configured=config.templatePack,customConfigured=Boolean(configured?.roles&&configured?.roleTemplates&&configured.id===value.packId);
        if(!pack&&!customConfigured)issues.push({field:'social.templateMatch.packId',code:'ENUM',message:'匹配模板包不存在'});
        else if(configured&&configured.id!==value.packId)issues.push({field:'social.templateMatch.packId',code:'MISMATCH',message:'匹配摘要必须与 templatePack 一致'});
      }
    }
