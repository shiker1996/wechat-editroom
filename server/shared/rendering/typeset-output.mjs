...
export function htmlPreservesStructure(markdown, html) {
  ...
  let styleStripped = String(html);
  let previous;
  do {
    previous = styleStripped;
    styleStripped = styleStripped.replace(/<style\b[\s\S]*?<\/style>/gi, '');
  } while (styleStripped !== previous);
  const visible = styleStripped.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
