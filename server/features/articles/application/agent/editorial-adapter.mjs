...
import { substantiveDecision, confirmedFactsDecision, researchBasisDecision, researchBasisValidationMessage } from '../../domain/editorial-readiness.mjs';
...
export const EDITORIAL_APPLICATION_TOOLS = Object.freeze([Object.freeze({
  ...
  inputSchema: {
    ...
    required: ['point_ids'],
    properties: {
      point_ids: { type: 'array', minItems: 1, maxItems: 3, uniqueItems: true, description: '从 research-selection-catalog 原样选择 1–3 个 point_id，不要自行构造、截断或改写。', items: { type: 'string', minLength: 1, maxLength: 160, description: '研判拓展点目录中的原样 point_id。' } },
      rationale: { type: 'string', maxLength: 500, description: '说明这些研判点如何支撑当前角度和命题。' },
    },
  },
...
export function editorialPreflightToolResult(result) {
  const warnings = (result?.gates || []).flatMap((item) => Array.isArray(item?.warnings) ? item.warnings : []).filter(Boolean).map(String);
  return {
    status: 'ok',
    data: result && typeof result === 'object' ? result : {},
    artifacts: [],
    warnings,
    provenance: { provider: 'editorial-agent', operation: 'editorial-preflight' },
  };
}
