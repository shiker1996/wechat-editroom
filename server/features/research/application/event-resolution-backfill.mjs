...
function fallbackLegacyClusters(hotspots = []) {
  const groups = new Map();
  for (const hotspot of hotspots) {
    let raw = {};
    try { raw = JSON.parse(hotspot.raw_json || '{}'); } catch {}
    const key = String(raw.aiTags?.eventKey || `singleton-${hotspot.id}`).trim().toLowerCase();
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(hotspot);
  }
  return [...groups.entries()].map(([key, members]) => ({
    event_id: `ELEGACY-${Buffer.from(key).toString('hex').slice(0, 12).toUpperCase()}`,
    articles: members.map((hotspot) => ({ hotspot_id: hotspot.id })),
  }));
}
...
export function runEventResolutionBackfill({ store, workspaceRoot, limit = 14, apply = false, clusterItems = fallbackLegacyClusters } = {}) {
