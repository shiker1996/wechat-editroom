...
export function extractSocialContentFeatures(row = {}, { sources = {} } = {}) {
  const directory = String(sources.directory || '');
  const copyContent = String(sources.copyContent || '');
  const cardPlan = sources.cardPlan || null;
  const layoutReport = sources.layoutReport || null;
  const stageExecutions = sources.stageExecutions || null;
...
export function buildSocialContentFeedbackSnapshot(rows = []) {
  ...
  const items = sourceRows.map((row) => ({ ...row, title: row.metric_title || row.artifact_title || '', features: extractSocialContentFeatures(row, { sources: row.social_content_sources }) }));
