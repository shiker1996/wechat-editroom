...
function localCandidates(selected, artifacts) {
  ...
  return (artifacts || []).filter((item) => {
    ...
    const sameArticle = selected && item.normalized_title === selected.normalized_title && item.article_date === selected.article_date;
  ...
  for (const match of matches) {
    try {
      ...
      failures.push({
        ...
        title: match.metric_title || '',
        error: error?.message || String(error),
      });
    }
  }
  return { total: matches.length, linked, social_copy: socialCopy, needs_external: needsExternal, failed, failures };
}
