...
- `project-discovery-feedback.mjs` 复用已确认 GitHub 图文的公众号指标，按项目场景、项目类型和仓库聚合相对表现，生成可审计的下一批项目发现排序偏置；必须人工确认后才生效，不修改文章或图文技能。
- `content-planning-recommendations.mjs` 将反馈快照和素材评估合成为可解释的软推荐，支持复盘优先排序、目标选择、标题结构提示和下一篇验证预告；不替代热点评分，也不自动修改账号长期策略。
- `wechat-strategy-recommendations.mjs` 在两个不同内容周期后生成只读账号策略草案，覆盖内容配比、分发比例和关注理由；建议必须由作者确认后手工写回账号配置。
