...
/**
 * Centralize visual literals in the editor/workflow stylesheet while keeping
 * user-authored theme data and inline SVG assets untouched.
 */
import fs from 'node:fs';
import path from 'node:path';

const root = process.cwd();
const sourcePath = path.join(root, 'public', 'styles', 'editor-themes.css');
...
function isInsideCustomPropertyReference(line, index) {
...
for (const line of lines) {
  ...
  while ((match = colorPattern.exec(line))) {
    if (isInsideCustomPropertyReference(line, match.index)) continue;
    ...
    if (!values.has(value)) values.set(value, '--editor-palette-' + slug(value));
  }
}

if (values.size === 0) {
  console.log('editor-themes.css 已没有可迁移的视觉硬编码颜色。');
  process.exit(0);
}
...
for (const line of lines) {
  ...
  migrated += line.replace(colorPattern, (value, ...args) => {
    ...
    if (isInsideCustomPropertyReference(line, index)) return value;
...
console.log('已将 editor-themes.css 中 ' + values.size + ' 种视觉颜色集中到主题变量。');
