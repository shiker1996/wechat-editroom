...
/** Centralize visual colors used by social-card and graphic production UI. */
import fs from 'node:fs';
import path from 'node:path';

const root = process.cwd();
const sourcePath = path.join(root, 'public', 'styles', 'social-card.css');
...
for (const line of lines) {
  ...
  while ((match = colorPattern.exec(line))) {
    ...
    if (!values.has(value)) values.set(value, '--social-palette-' + slug(value));
  }
}

if (values.size === 0) {
  console.log('social-card.css 已没有可迁移的视觉硬编码颜色。');
  process.exit(0);
}
...
const marker = '  /* Generated social palette tokens. */';
if (tokenSource.includes(marker)) throw new Error('社交卡片主题变量块已存在，请人工检查后再运行迁移。');
...
console.log('已将 social-card.css 中 ' + values.size + ' 种视觉颜色集中到主题变量。');
