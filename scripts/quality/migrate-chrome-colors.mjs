...
/** Centralize visual colors used by the desktop application chrome. */
import fs from 'node:fs';
import path from 'node:path';

const root = process.cwd();
const sourcePath = path.join(root, 'public', 'styles', 'chrome.css');
...
for (const line of lines) {
  ...
  while ((match = colorPattern.exec(line))) {
    ...
    if (!values.has(value)) values.set(value, '--chrome-palette-' + slug(value));
  }
}

if (values.size === 0) {
  console.log('chrome.css 已没有可迁移的视觉硬编码颜色。');
  process.exit(0);
}
...
const marker = '  /* Generated chrome palette tokens. */';
if (tokenSource.includes(marker)) throw new Error('桌面壳主题变量块已存在，请人工检查后再运行迁移。');
...
console.log('已将 chrome.css 中 ' + values.size + ' 种视觉颜色集中到主题变量。');
